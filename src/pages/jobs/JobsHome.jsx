...
const jobListings = [
  { id: 1, title: 'Lead UI/UX Designer', company: 'Google', location: 'Bangalore', type: 'Full-time', salary: '₹28-45 LPA', tags: ['Figma', 'Strategy'], posted: '2d ago', color: '#3B82F6', description: "Design intuitive user interfaces for Google's next-gen products." },
  { id: 2, title: 'Senior Frontend Dev', company: 'Microsoft', location: 'Hyderabad', type: 'Full-time', salary: '₹35-52 LPA', tags: ['React', 'Next.js'], posted: '1d ago', color: '#10B981', description: 'Build performant web applications using React and TypeScript.' },
  { id: 3, title: 'Senior Data Analyst', company: 'Amazon', location: 'Remote', type: 'Contract', salary: '₹18-28 LPA', tags: ['Python', 'Tableau'], posted: '3d ago', color: '#F59E0B', description: 'Analyze large datasets to drive business decisions.' },
  { id: 4, title: 'Growth Product Lead', company: 'Flipkart', location: 'Bangalore', type: 'Full-time', salary: '₹40-60 LPA', tags: ['Product', 'Growth'], posted: '5d ago', color: '#db2777', description: 'Lead product strategy and roadmap for consumer-facing features.' },
  { id: 5, title: 'AI/ML Researcher', company: 'Zomato', location: 'Delhi', type: 'Full-time', salary: '₹45-75 LPA', tags: ['PyTorch', 'GenAI'], posted: '12h ago', color: '#630ed4', description: 'Build and deploy machine learning models for recommendation systems.' },
  { id: 6, title: 'Technical Writer', company: 'Swiggy', location: 'Remote', type: 'Part-time', salary: '₹12-18 LPA', tags: ['API', 'Docs'], posted: '4d ago', color: '#EF4444', description: 'Create engaging content for blogs, social media, and marketing.' },
];

const aiTools = [
  { to: '/jobs/resume-analyzer', icon: Target, title: 'ATS Scanner', desc: 'Resume match score', color: '#7c3aed', bg: '#f5f3ff' },
  { to: '/jobs/skill-translator', icon: Zap, title: 'Skill Translator', desc: 'Skills into income', color: '#3b82f6', bg: '#eff6ff' },
  { to: '/jobs/career-simulator', icon: TrendingUp, title: 'Career Simulator', desc: '6-month roadmap', color: '#10B981', bg: '#ecfdf5' },
  { to: '/jobs/marketplace', icon: Store, title: 'Marketplace', desc: 'Women-owned biz', color: '#db2777', bg: '#fdf2f8' },
  { to: '/jobs/exchange', icon: MapPin, title: 'Job Exchange', desc: 'Hyperlocal tasks', color: '#d97706', bg: '#fffbeb' },
];
  ...
  return (
    <div>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div style={{ position: 'absolute', bottom: '-40px', left: '-30px', width: '160px', height: '160px', background: 'rgba(99,102,241,0.06)', borderRadius: '50%', filter: 'blur(60px)', pointerEvents: 'none' }} />
        <div style={{ position: 'relative', zIndex: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '16px' }}>
            <div style={{ width: '52px', height: '52px', borderRadius: '16px', background: 'linear-gradient(135deg, #3B82F6, #6366f1)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 6px 20px rgba(59,130,246,0.25)' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {[{ l: '128 active', c: '#3b82f6', bg: '#eff6ff', bd: '#dbeafe', I: Briefcase }, { l: '5 matches', c: '#10B981', bg: '#ecfdf5', bd: '#d1fae5', I: Sparkles }, { l: 'Trending', c: '#d97706', bg: '#fffbeb', bd: '#fef3c7', I: TrendingUp }].map((b, i) => (
        ...
        {filteredJobs.map((job, i) => (
          <motion.div key={job.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
            ...
            <div style={{ padding: '16px 18px', display: 'flex', alignItems: 'flex-start', gap: '14px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: `${job.color}15` }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                  <span style={{ padding: '2px 8px', borderRadius: '6px', background: '#eff6ff', color: '#2563eb', fontSize: '11px', fontWeight: 700 }}>{job.type}</span>
                  <span style={{ padding: '2px 8px', borderRadius: '6px', background: '#ecfdf5', color: '#059669', fontSize: '11px', fontWeight: 700 }}>{job.salary}</span>
              ...
              <ChevronRight size={18} style={{ color: expanded === job.id ? '#3b82f6' : 'var(--color-outline-variant)', flexShrink: 0, transform: expanded === job.id ? 'rotate(90deg)' : 'none', transition: 'transform 0.2s' }} />
            </div>
            <AnimatePresence>
              {expanded === job.id && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                  ...
                  <div style={{ padding: '16px 18px' }}>
                    <div style={{ padding: '14px', borderRadius: '12px', background: 'var(--color-surface-low)', marginBottom: '14px' }}>
                      <p style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#d97706', marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '6px' }}><Sparkles size={11} />About the role</p>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button onClick={(e) => { e.stopPropagation(); if (!applied.includes(job.id)) handleApplyClick(job); }} disabled={applied.includes(job.id)}
                        style={{ flex: 1, padding: '12px', borderRadius: '12px', border: 'none', fontSize: '13px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', fontFamily: 'var(--font-sans)', background: applied.includes(job.id) ? '#ecfdf5' : 'linear-gradient(135deg, #3B82F6, #6366f1)', color: applied.includes(job.id) ? '#059669' : 'white', boxShadow: applied.includes(job.id) ? 'none' : '0 4px 12px rgba(59,130,246,0.25)' }}>
                      ...
                      <button onClick={(e) => { e.stopPropagation(); handleSave(job.id); }}
                        style={{ padding: '12px 16px', borderRadius: '12px', border: '1px solid var(--color-surface-container)', background: saved.includes(job.id) ? '#fff1f2' : 'var(--color-surface-lowest)', cursor: 'pointer', display: 'flex', alignItems: 'center', color: saved.includes(job.id) ? '#f43f5e' : 'var(--color-outline)' }}>
            ...
            <motion.div initial={{ y: '100%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '100%', opacity: 0 }}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', minWidth: 0 }}>
                  <div style={{ width: '44px', height: '44px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: `${applyingJob.color}15` }}>
                    <Building2 size={20} style={{ color: applyingJob.color }} />
                  </div>
                  <div style={{ minWidth: 0 }}>
                    <p style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#3b82f6', margin: '0 0 2px' }}>Applying for</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginBottom: '20px' }}>
                ...
                <div>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', borderRadius: '12px', border: '2px dashed #93c5fd', background: '#eff6ff40', cursor: 'pointer' }}>
                    <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#3b82f6', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}><Upload size={18} /></div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {resume && <CheckCircle size={18} style={{ color: '#10B981', flexShrink: 0 }} />}
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button onClick={handleConfirmApply} style={{ flex: 1, padding: '12px', borderRadius: '12px', background: 'linear-gradient(135deg, #3B82F6, #6366f1)', color: 'white', border: 'none', fontSize: '13px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', fontFamily: 'var(--font-sans)', boxShadow: '0 4px 12px rgba(59,130,246,0.25)' }}>
