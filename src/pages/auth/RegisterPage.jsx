...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      toast.success('Welcome to SHAKTI AI! 🎉');
      navigate('/');
    } catch (err) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: 'radial-gradient(circle, rgba(99,14,212,0.3) 0%, transparent 70%)',
      }} />
      <div style={{
        position: 'absolute', bottom: '-20%', right: '-10%', width: '600px', height: '600px',
        borderRadius: '50%', filter: 'blur(120px)', pointerEvents: 'none', opacity: 0.12,
      ...
      >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '24px' }}>
            <div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                />
                <User size={16} style={{ position: 'absolute', right: '16px', top: '50%', transform: 'translateY(-50%)', color: 'var(--color-outline)' }} />
              </div>
            </div>
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <input type="password" value={confirmPassword}
                    ...
                    onBlur={(e) => { e.target.style.borderColor = 'transparent'; e.target.style.background = 'var(--color-surface-low)'; }}
                  />
                  <Lock size={16} style={{ position: 'absolute', right: '16px', top: '50%', transform: 'translateY(-50%)', color: 'var(--color-outline)' }} />
                </div>
              </div>
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          ...
          <button onClick={handleDemoSignUp} disabled={loading}
            style={{
              padding: '14px', borderRadius: '14px', border: '1px solid var(--color-surface-container)',
              background: 'var(--color-surface-lowest)', cursor: 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px',
        ...
        <p style={{ textAlign: 'center', marginTop: '28px', fontSize: '14px', color: 'var(--color-outline)' }}>
          Already a member?{' '}
          <Link to="/login" style={{ fontWeight: 700, color: 'var(--color-shakti-primary)' }}>
            Sign In
          </Link>
        </p>
      </motion.div>
